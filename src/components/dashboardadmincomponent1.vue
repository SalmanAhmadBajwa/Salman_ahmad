                  ...
                  >
                    <template v-slot:activator="{ on, attrs }">
                      <v-btn
                        ...
                        v-bind="attrs"
                        v-on="on"
                        @click="all_contract_list(itemgroup.group_id)"
                      >
...
//    }
   for (var i = 0; i < x.length; i++) {
    ...
    groupsetting(group, property) {
      ...
      else if (property.title === "Kopie erstellen")   console.log("Fff");
      {
      this.duplicategroup(store.state,group.group_id);
  ...
  //    this.$store.commit("duplicategroup",group_id);
      duplicategroup(state,value)
      {
        ...
        var group = JSON.parse(JSON.stringify(state.group.find(o=>o.group_id === value)));
        group.group_id =  ++state.group_counter;
        console.log(state.group_counter);
        var d1 = new Date();
