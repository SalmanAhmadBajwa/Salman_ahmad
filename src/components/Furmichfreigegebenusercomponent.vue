<template>
  <v-content class="ma-0 pa-0">
      <v-row>
        ...
        >
         Fur mich freigegeben 
          <v-btn icon @click="liston=!liston">
            <v-icon v-if="liston" color="black" size="28"> mdi-menu-down</v-icon>
            <v-icon v-if="!liston" color="black" size="28"> mdi-menu-right</v-icon>
          </v-btn>
        </p>
          </v-col>
      </v-row>
 ...
 v-if="liston"
  ...
  <template v-slot:no-data>
    ...
    <template v-slot:[`item.front_title`]="{ item }">
      ...
      @click="currentusercontracts(item.title,item.group_title,item.front_title)"
    ...
    <template v-slot:[`item.time`]="{ item }" >
      <p class="mt-4">{{item.date}} | {{item.time}}</p>
    </template>
    <template v-slot:[`item.percentage`]="{ item }">
      <p class="mt-4">{{item.status}} | {{item.percentage}}</p>
    </template>
    <template v-slot:[`item.group_title`]="{ item }">
      <p class="mt-4">{{item.group_title}}</p>
    </template>
 </v-data-table>
  </v-content>
</template>
...
export default {
name: "Furmichfreigegebenusercomponent",
data: () => ({
  headers:[
    {
      text: "Objekt | Kunde",
  ...
  computed: {
     ...
     "Papierkorbadmin","Dashboardadmin","grid","list","Zuletztbearbeitetuser",
     "Furmichfreigegebenuser","Papierkorbuser",]),
    rules() {
      return [
  ...
  mounted() {
    ...
    if(this.isAdmin === true){
      ...
      let val = JSON.parse(
        localStorage.getItem("contracts of " + this.current_user_name)
      );
      // console.log(val);

      this.contractcheckinguser = Object.keys(val);
      console.log(this.contractcheckinguser);
    }
  },
  //methods
  methods: {
