  ...
  <v-content class="ma-0 pa-0">
   <!-- 1st heading -->
          <div style="display: flex" class="mb-5">
            <!-- <h4>{{current_admin_name}}</h4> -->
            <hp style="font-family:railway;font-size:18px;font-weight:690;">Zuletzt bearbeitet</hp>
            <v-icon color="black" size="28" class=" ml-2" >
              mdi-menu-down
            </v-icon>
          </div>
          ...
          <div style="display: flex" >
            <div
              v-for="(item, index) in current_admin_recent_contracts"
              :key="index"
            >
              <v-card
                class="mr-4 mt-2"
              ...
              <p class="mt-n6 ml-9" style="color: white; font-size: 12px">
                ...
                >
                  <div style="display: flex">
                    <v-icon class="mt-n3 ml-2" size="14">mdi-account-outline</v-icon>
                        
                    <p 
                    v-if="contractchecking.includes(item.contract_front_title +'-'+ item.group_title)"
                    class="pt-1 pl-1" style="font-size: 12px">
                      {{ item.contract_front_title }} {{item.group_title}}
                    </p>
                    <p 
                    v-if="!contractchecking.includes(item.contract_front_title +'-'+ item.group_title)"
                    class="pt-1 pl-1" style="font-size: 12px">
                      {{ item.contract_title }}_original {{item.group_title}}
                    </p>
...
export default {
    name: "zuletztbearbeitetadmincomponent",
    data: () => ({
    auto_users: [],
    group_title: "",
    checked: true,
